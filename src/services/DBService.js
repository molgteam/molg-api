...
const fetchWorkers = async () => {
  try {
    let workers = await knex('account').where({ isWorked: 1 });
    return workers;
  } catch (e) {
    throw new Error('찌리리리리지ㅣ지지지 으아아악 감전이다.', e);
  }
...
const fetchWorkerByRandom = async () => {
  try {
    let worker = await knex.raw(
      `SELECT * FROM account WHERE isWorked = '1' ORDER BY RAND() LIMIT 1`,
    );
    return worker[0][0];
  } catch (e) {
    throw new Error('찌리리리리지ㅣ지지지 으아아악 감전이다.', e);
...
const updateWorkerErrorCnt = async (workerUsername) => {
  try {
    await knex.raw(
      `UPDATE account SET errorCnt = errorCnt + 1 WHERE username = '${workerUsername}'`,
    );
  } catch (e) {
    throw new Error('찌리리리리지ㅣ지지지 으아아악 감전이다.', e);
  }
